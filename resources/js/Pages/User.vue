...
const props = defineProps({ postsByUser: Object, user: Object, status: Boolean, followersCount : Number, followingsCount : Number  })
const { postsByUser, user, status, followersCount } = toRefs(props)


const stat = ref(status.value);
const ButtonTextFollow = computed(() => {
    return stat.value ? 'Unfollow' : 'Follow'
})
...
function followUser(id) {
    axios.post(`/users/${id}/follow`)
        .then(res => stat.value = !stat.value)
}
            ...
            <div class="flex items-center md:justify-between">
                ...
                <label for="fileUser">
                    ...
                    <svg v-else
                         ...
                         class="inline-block md:w-[200px] w-[100px] rounded-full text-gray-300 bg-gray-100 dark:bg-gray-600"
                         fill="currentColor"
                         viewBox="0 0 24 24">
                ...
                <div class="ml-6 w-full">
                    <div class="flex items-center md:mb-8 mb-5">
                        ...
                        <button  v-if="user.id !== $page.props.auth.user.id" @click.prevent="followUser(user.id)"   class="md:block hidden md:mr-6 p-1 px-4 rounded-lg text-[16px] font-extrabold bg-sky-300 hover:bg-gray-200">
                                {{ButtonTextFollow}}
                        </button>
<!--                        <Link  v-if="user.id !== $page.props.auth.user.id" :href="route('users.show', { id: post.user.id })" class="md:block hidden md:mr-6 p-1 px-4 rounded-lg text-[16px] font-extrabold bg-sky-300 hover:bg-gray-200">-->
<!--                            Send-->
<!--                        </Link>-->

                        <button  v-if="user.id !== $page.props.auth.user.id" @click.prevent="store(user.id)" class="md:block hidden md:mr-6 p-1 px-4 rounded-lg text-[16px] font-extrabold bg-sky-300 hover:bg-gray-200">
                            Send Message
                        </button>
...
<!--                    </button>-->
                    <div class="md:block hidden">
                        <div class="flex items-center text-[18px]">
                            ...
                            <div class="mr-6">
                                <span class="font-extrabold">{{ followersCount }}</span>  followers
                            </div>
                            <div class="mr-6">
                                <span class="font-extrabold">{{ followingsCount }}</span>  following
                            </div>
                        </div>
                    </div>
                </div>
